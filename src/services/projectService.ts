...
import type { CreateProjectRequest, ProjectResponse, Project, ProjectMember, UpdateProjectRequest } from '../types/project';
...
export const projectService = {
    ...
    createProject: async (data: CreateProjectRequest): Promise<ProjectResponse> => {
        try {
            const response = await api.post<ProjectResponse>('/api/projects/create', data);
            return response.data;
        } catch (error) {
    ...
    deleteProjectMember: async (projectId: number, userId: number): Promise<void> => {
        try {
